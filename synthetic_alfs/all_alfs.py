...
def ensure_target_table():
    """
    Create target table for storing distinct alf_e values
    """
    columns_sql = """
        "alf_e" BIGINT
    """
...
def get_distinct_alfs_from_table(table_name, chunk_size=CHUNK_SIZE):
    ...
    try:
        ...
        while offset < total_distinct:
            ...
            # Progress update
            print(f"Collected {len(all_alfs)} unique alf_e values so far from {table_name}")
        
        print(f"Completed collection from {table_name}: {len(all_alfs)} unique alf_e values")
        return all_alfs
...
def collect_all_distinct_alfs(table_names):
    ...
    for table_name in table_names:
        try:
            ...
            print(f"Total unique alf_e values collected so far: {len(all_distinct_alfs)}")
        except Exception as e:
            print(f"Error collecting from {table_name}: {e}")
            continue
    
    print(f"Final collection complete: {len(all_distinct_alfs)} total unique alf_e values")
